...
Citizen.CreateThread(function()
	while true do
		...
		if DoesEntityExist( ped ) and not IsEntityDead( ped ) and not IsPedInAnyVehicle(PlayerPedId(), true) and not IsEntityInWater(ped) and not IsPedSwimmingUnderWater(ped) and not IsPedSwimming(ped) and CheckSkin(ped) then
			if not IsPauseMenuActive() and not IsEntityPlayingAnim(ped, "move_crawl", "onfront_fwd", 3) and not IsEntityPlayingAnim(ped, "move_crawl", "onfront_bwd", 3) and not IsEntityPlayingAnim(ped, "amb@lo_res_idles@", "lying_face_down_lo_res_base", 3) and not IsEntityPlayingAnim(ped, "amb@lo_res_idles@", "lying_face_up_lo_res_base", 3) then 
